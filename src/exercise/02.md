...
Declarative X Imperative languages:
- Declarative use imperative commands;
- Declarative describes the final output expected while imperative tells the necessary steps to get the final output
- Javascript might be used as declarative or imperative depending on the context. Functions such as `map` and `reduce` are declarative.
Difference between React and React DOM:
- React creates the element
- React DOM renders a created element on the screen
